...
description: Constrain supervisor delegation, retry budgets, and stage ordering for offline indexing. Use when the main agent must decide which subagent to call next, how many task rounds remain, and when to stop and surface an incomplete workflow.
---
...
5. If audit shows no progress across retries, stop delegating and signal an incomplete workflow.
...
- Prefer an explicit stop over speculative extra delegation once the retry budget is exhausted.
