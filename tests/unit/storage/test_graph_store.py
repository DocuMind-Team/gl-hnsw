...
def test_discovery_service_adds_mirror_edges_for_symmetric_relations(test_root):
    ...
    service = LogicDiscoveryService(
        orchestrator=None,
        brief_store=BriefStore(test_root / "data" / "briefs"),
        graph_store=store,
        anchor_memory_store=AnchorMemoryStore(test_root / "data" / "anchor_memory"),
        semantic_memory_store=SemanticMemoryStore(
            test_root / "data" / "memory" / "entities.json",
            test_root / "data" / "memory" / "relations.json",
            test_root / "data" / "memory" / "rejections.json",
        ),
        graph_memory_store=GraphMemoryStore(test_root / "data" / "memory" / "graph_stats.json"),
