...
from hnsw_logic.services.beir_eval import _should_build_offline_graph, evaluate_beir_dataset, load_beir_dataset, prepare_beir_work_root
...
def test_evaluate_beir_dataset_persists_report(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("hnsw_logic.services.beir_eval.download_and_extract_beir_dataset", lambda *_args, **_kwargs: dataset_root)
    monkeypatch.setattr("hnsw_logic.services.beir_eval.build_app", lambda _work_root: FakeApp())
