...
class OpenAICompatibleProvider(OpenAIProviderTransportMixin, StubProvider):
    """Live provider that delegates profiling and judgment to remote models."""
    ...
    def __init__(self, config: ProviderConfig, root_dir: Path | None = None):
        ...
        self._local_embedding_tokenizer: Any | None = None
    ...
    def _init_local_bge_m3(self) -> None:
        ...
        if self._local_embedding_model is None:
            raise RuntimeError("Failed to initialize the local BGE-M3 model")
    ...
    def _brief_vectors(self, briefs: list[DocBrief]) -> np.ndarray:
        ...
        if missing_texts:
            encoded = (
                self._embed_local_bge_m3(missing_texts)
                if self._local_embedding_model is not None
                else np.asarray(self._embedding_client().embed_documents(missing_texts), dtype=np.float32)
            )
    ...
    def _embedding_client(self) -> Any:
        if self._embeddings is None:
            raise RuntimeError("Remote embedding client is not configured")
    ...
    def _embed_local_bge_m3(self, texts: list[str]) -> np.ndarray:
        ...
        if self._local_embedding_model is None or self._local_embedding_tokenizer is None:
            raise RuntimeError("Local BGE-M3 embedding components are not initialized")
    ...
    def embed_texts(self, texts: Iterable[str]) -> np.ndarray:
        ...
        vectors = self._embedding_client().embed_documents(items)
