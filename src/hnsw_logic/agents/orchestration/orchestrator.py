...
from hnsw_logic.embedding.providers.types import CandidateProposal, JudgeSignals
...
class LogicOrchestrator:
    ...
    relation_judge: Any
    memory_curator: Any
    edge_reviewer: Any = None
    deepagent: object | None = None
    retrieval_config: RetrievalConfig = field(default_factory=RetrievalConfig)
    _embedding_cache: dict[str, Any] = field(default_factory=dict)
    _signal_runtime: SkillSignalRuntime = field(default_factory=SkillSignalRuntime)
    ...
    def _signal_bundle(self, anchor: DocBrief, candidate: DocBrief, metrics: dict[str, float], fit_scores: dict[str, float], relation_type: str) -> JudgeSignals:
        ...
        if duplicate_penalty >= 0.34 and float(signal_report.get("contrast_evidence", 0.0) or 0.0) < 0.56:
    ...
    def _select_diverse_proposals(
        ...
        for _score, _relation_type, proposal in sorted(
            scored,
            key=lambda item: (-item[0], item[1], item[2].doc_id),
        ):
